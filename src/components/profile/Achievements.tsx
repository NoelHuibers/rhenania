...
import { getUserAchievements, type UserAchievementData } from "~/server/actions/profile/achievements";
...
function AchievementCard({ a }: { a: UserAchievementData }) {
  ...
  return (
    ...
    >
      <div className="text-2xl shrink-0 w-9 text-center">
        {isSecret ? "🔒" : (a.icon ?? "🏆")}
      </div>
      <div className="min-w-0 flex-1 space-y-1">
        <div className="flex items-start justify-between gap-2">
          ...
          <div className="flex items-center gap-1 shrink-0">
            ...
            <Badge className={`text-[10px] px-1.5 py-0 ${RARITY_STYLES[a.rarity]}`}>
      ...
      <CardHeader>
        <div className="flex items-center justify-between">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            /{data.length} ·{" "}
            <span className="font-semibold text-foreground">{totalPoints}</span> Punkte
          </div>
        </div>
      </CardHeader>
