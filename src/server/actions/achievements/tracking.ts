...
async function processAchievement(
  ...
  switch (achievement.key) {
    ...
    // ── Special drinks ─────────────────────────────────────────────────
    case "kaisersflotte":
      if (event === "order_created" && context && /sekt/i.test(context.drinkName)) {
        newValue = 1;
      }
      break;
...
async function getDrinkCountByName(
  ...
  const result = await db
    ...
    .from(orders)
    .where(
      and(eq(orders.userId, userId), like(orders.drinkName, `%${namePattern}%`))
    );
  return result[0]?.total ?? 0;
}
...
async function getHellPilsRatio(
  ...
  const [hellResult, pilsResult] = await Promise.all([
    ...
    db
      ...
      .from(orders)
      .where(
        and(eq(orders.userId, userId), like(orders.drinkName, "%Pils%"))
      ),
  ]);
...
async function getMorningBeerCount(userId: string): Promise<number> {
  const now = new Date();
  const from = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 5, 0, 0);
  const to = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 8, 0, 0);
  const result = await db
    .select({ total: sql<number>`coalesce(sum(${orders.amount}), 0)` })
    .from(orders)
    .where(
...
async function getOrdersInLastHour(userId: string): Promise<number> {
  ...
  const result = await db
    ...
    .from(orders)
    .where(
      and(eq(orders.userId, userId), gte(orders.createdAt, oneHourAgo))
    );
  return result[0]?.count ?? 0;
}
...
async function getMidnightBeerCount(userId: string): Promise<number> {
  ...
  const to = new Date(now.getFullYear(), now.getMonth(), now.getDate(), 1, 0, 0);
  const result = await db
    .select({ total: sql<number>`coalesce(sum(${orders.amount}), 0)` })
    .from(orders)
    .where(
...
async function getBestWeeklyGameStreak(userId: string): Promise<number> {
  ...
  const days = [
    ...new Set(
      allGames.map((g) => g.playedAt.toISOString().split("T")[0]!)
    ),
  ].sort();
...
export async function recalculateAllAchievements(userId: string): Promise<void> {
  const [allAchievements, alreadyUnlocked] = await Promise.all([
    db.select().from(achievements).where(eq(achievements.isActive, true)),
    db
      .select({ achievementId: userAchievements.achievementId })
